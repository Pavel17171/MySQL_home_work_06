...
BEGIN
    ...
    WHERE u.id IN (
		(
			...
			WHERE target_user_id = find_user_id AND `status` = 'approved'
			UNION
			SELECT target_user_id
			FROM friend_requests
			WHERE initiator_user_id = find_user_id AND `status` = 'approved'
		)
		UNION 
		(
			SELECT user_id
		...
		(
			...
			WHERE target_user_id IN (		
				...
				FROM friend_requests
				WHERE initiator_user_id = find_user_id AND `status` = 'approved'
				) AND initiator_user_id != find_user_id
			UNION
			SELECT target_user_id
			FROM friend_requests
			WHERE initiator_user_id IN (		
				...
				FROM friend_requests
				WHERE initiator_user_id = find_user_id AND `status` = 'approved'
				) AND target_user_id != find_user_id
		)
	)	
	ORDER BY RAND()
	LIMIT 5
...
CALL friends(1); -- в скобках вводим id пользователя, для которого перебираем условия
...
BEGIN 
    ...
    SET count_friends = 
        ...
        WHERE u.id IN 
            ...
            WHERE fr.target_user_id = find_user_id 
				AND `status` = 'approved'
			UNION 
            SELECT fr.target_user_id 
            FROM friend_requests fr 
            WHERE fr.initiator_user_id = find_user_id 
				AND `status` = 'approved'
			)
		);
    SET count_users = (SELECT COUNT(id) FROM users);
RETURN
	count_friends / count_users;
END $$
...
BEGIN 
	...
	SET answer =
		CASE
			WHEN time_now BETWEEN '00:00:00' AND '05:59:59'
				THEN 'Доброй ночи'
			WHEN time_now BETWEEN '06:00:00' AND '11:59:59'
				THEN 'Доброе утро'
			WHEN time_now BETWEEN '12:00:00' AND '17:59:59'
				THEN 'Добрый день'
			WHEN time_now BETWEEN '18:00:00' AND '23:59:59'
				THEN 'Добрый вечер'
		END;
RETURN
       answer;
...
/*
4. (по желанию)* Создайте таблицу logs типа Archive. 
...
CREATE TABLE `logs` 
	(
		...
		name_table VARCHAR(15) NOT NULL,
        id_prim_key INT UNSIGNED NOT NULL
    ) 
    ENGINE = ARCHIVE;
...
INSERT INTO `logs` SET name_table = 'users', id_prim_key = NEW.id;

DROP TRIGGER IF EXISTS log_communities;
CREATE TRIGGER log_communities
AFTER INSERT ON communities FOR EACH ROW
INSERT INTO `logs` SET name_table = 'communities', id_prim_key = NEW.id;

DROP TRIGGER IF EXISTS log_messages;
CREATE TRIGGER log_messages
AFTER INSERT ON messages FOR EACH ROW
INSERT INTO `logs` SET name_table = 'messages', id_prim_key = NEW.id;

-- Для проверки:
INSERT INTO `users` (`id`, `firstname`, `lastname`, `email`, `phone`) VALUES ('101', 'R', 'N', 'ar0@example.org', '9374071116');
INSERT INTO `messages` VALUES ('101','5','1','V..','2000-08-28 22:44:29');
INSERT INTO `communities` VALUES (101,'atque');
